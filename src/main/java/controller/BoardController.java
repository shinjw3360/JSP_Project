...
public class BoardController extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "update":
			try {
				...
				log.info("{}",b);
				isOk = bsv.update(b);
				response.sendRedirect("details?bno="+bno);
			} catch (Exception e) {
				e.printStackTrace();
			}
			break;
	...
	private void resForword(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// forward : 처리주체가 WAS (톰캣) => 처리 속도가 빠름
		// => 처음 요청 url이 변경되지 않음
		// /brd/insert 고정되어서 새로고침시 계속 추가
		// request.getRequestDispatcher("list").forword(request, response);
		
		// redirect : 처리주체가 Web Browser (was에서 받은 response로 새로운 URL로 다시 Reguest)
				
	}


	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// Get으로 오는 요청에 대한 처리를 하고, HTML 파일을 생성하여 response 객체를 생성하여 jsp 로 전송
